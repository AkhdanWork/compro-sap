                ...
                <form action="{{ route('admin.partner.update', $partnerContent->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $partnerContent->title) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="order" id="order" value="{{ old('order', $partnerContent->order) }}" min="0"
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm" required>
                            <p class="mt-1 text-sm text-gray-500">Current order: <strong>{{ $partnerContent->order }}</strong></p>
                        </div>
                        ...
                        <div>
                            ...
                            @if($partnerContent->image)
                                <div class="mb-4">
                                    ...
                                    <div class="relative inline-block">
                                        <img src="{{ asset('storage/'.$partnerContent->image) }}" alt="{{ $partnerContent->title }}" 
                                             class="w-48 h-32 rounded-lg object-contain border-2 border-gray-300 shadow-sm bg-gray-50 p-2">
                                        <div class="absolute top-2 right-2 bg-green-500 text-white px-2 py-1 rounded text-xs">Current</div>
                                    </div>
                                </div>
                            @endif
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                @if($partnerContent->image) Upload new logo to replace current one. @endif
                            ...
                            <div id="imagePreview" class="mt-4 hidden">
                                ...
                                <div class="relative inline-block">
                                    <img src="" alt="Preview" class="w-48 h-32 rounded-lg object-contain border-2 border-red-300 shadow-sm bg-gray-50 p-2">
                                    <div class="absolute top-2 right-2 bg-red-500 text-white px-2 py-1 rounded text-xs">New</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3 mt-8 pt-6 border-t border-gray-200">
                        <button type="submit" class="px-6 py-3 bg-red-500 border border-red-500 text-white rounded-md hover:bg-transparent hover:text-red-500 transition font-semibold">
                            <i class="bi bi-save mr-2"></i> Update Partner
                        </button>
