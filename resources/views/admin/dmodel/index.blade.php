        ...
        <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
            <div class="container mx-auto p-4">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold">Manage 3D Model Content</h1>
                    <a href="{{ route('admin.dmodel.create') }}" class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition inline-flex items-center">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-6">
                    ...
                    <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow border border-gray-200">
                        ...
                        <div class="p-4">
                            ...
                            <div class="flex gap-2 mt-3 pt-3 border-t border-gray-200">
                                <a href="{{ route('admin.dmodel.edit', $content->id) }}" 
                                   class="flex-1 bg-yellow-500 text-white py-2 rounded hover:bg-yellow-600 transition text-center text-sm">
                                    <i class="bi bi-pencil"></i> Edit
                                </a>
                                <button
                                    class="delete-btn flex-1 bg-red-500 text-white py-2 rounded hover:bg-red-600 transition text-sm"
                                    data-id="{{ $content->id }}"
                                    data-title="{{ $content->title }}"
                                    data-image="{{ $content->image ? asset('storage/'.$content->image) : '' }}"
                                >
                    ...
                    <div class="col-span-full text-center py-12">
                        ...
                        <p class="text-gray-500 text-lg">No 3D Model content found.</p>
                        <a href="{{ route('admin.dmodel.create') }}" 
                           class="inline-block mt-4 bg-red-500 text-white px-6 py-2 rounded hover:bg-red-600 transition">
                            Add Your First 3D Model Content
                        </a>
                    </div>
                    @endforelse
                </div>
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3 text-center">
                ...
                <h3 class="text-lg leading-6 font-medium text-gray-900 mb-3">
                    Delete 3D Model Content
                </h3>

                <div class="mb-4">
                    <img id="deleteImage"
                        class="mx-auto h-32 object-contain rounded bg-gray-100 hidden">
                    <p id="deleteTitle" class="mt-2 font-semibold text-red-600"></p>
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.delete-btn').forEach(button => {
                button.addEventListener('click', function () {
                    ...
                    const imageEl = document.getElementById('deleteImage');

                    form.action = `/admin/dmodel/${id}`;
